...
- **Resource-aware validation**: Do not run `pnpm build:check` by default; it is too resource-intensive for routine work. Prefer the lightest validation that matches the change, and only run heavier checks when the user explicitly asks.
...
- **Build Check**: `pnpm build:check` — Optional heavyweight validation; only run when the user explicitly asks for it
